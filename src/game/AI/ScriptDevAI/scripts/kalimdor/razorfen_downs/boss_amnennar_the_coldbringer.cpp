...
struct boss_coldbringerAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (!Spectrals70 && m_creature->GetHealthPercent() < 70)
        {
        ...
        if (!Spectrals55 && m_creature->GetHealthPercent() < 55)
        {
        ...
        if (!Hp && m_creature->GetHealthPercent() < 50)
        {
        ...
        if (!Spectrals30 && m_creature->GetHealthPercent() < 30)
        {
            DoScriptText(SAY_SUMMON30, m_creature);
            DoCastSpellIfCan(m_creature, SPELL_FROST_SPECTRES);
            Spectrals30 = true;
...
CreatureAI* GetAI_boss_coldbringer(Creature* pCreature)
{
